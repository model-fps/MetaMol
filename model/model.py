...
import numpy as np
import json
from model.util import RegressorBase
from model.maml import FCNReLUNormNetwork

class ModelRegressor(RegressorBase):
    def __init__(self, input_shape, args):
        super(ModelRegressor, self).__init__(input_shape, args)
    ...
    def forward(self, data_batch, epoch, num_steps, is_training_phase, **kwargs):
        xs, ys, splits, target_assay_idxes, target_assay_weight, _ = data_batch
        total_losses = []
        ...
        for x_task, y_task, split, target_assay_idx in zip(xs, ys, splits, target_assay_idxes):
            ...
            task_losses = []
            names_weights_copy, support_loss_each_step, task_losses = self.inner_loop(x_task, y_task, target_assay_idx,
                                                                                          split,
                                                                                          is_training_phase, epoch,
                                                                                          num_steps)
            target_loss, target_preds = self.net_forward(x=x_task,
                                                         y=y_task,
                                                         target_assay_idx=target_assay_idx,
                                                         split=split,
        ...
        losses = self.get_across_task_loss_metrics(total_losses=total_losses,
                                                   loss_weights=target_assay_weight)
        return losses, per_task_target_preds, final_weights, per_task_metrics

    def net_forward(self, x, y, split, weights, backup_running_statistics, training, num_step, target_assay_idx=None,
        ...
        if is_support:
            ...
            sup_sim_mat = self.cossim_matrix(sup_x, sup_x) - torch.eye(sup_num).cuda()
            ...
            sup_y_repeat = sup_y.unsqueeze(-1).repeat(1, sup_num)
            ...
            cross_sim_mat = self.cossim_matrix(sup_x, tgt_x)
            ...
            if use_embedding:
                ...
                sup_y_repeat = sup_y.unsqueeze(-1).repeat(1, tgt_num)
            ...
            tgt_sim_mat = self.cossim_matrix(tgt_x, tgt_x) - torch.eye(tgt_num).cuda()
